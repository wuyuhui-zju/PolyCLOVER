...
class TripletTransformer(nn.Module):
    ...
    def forward(self, g, triplet_h, dist_attn, path_attn):
        ...
        g.edata['a'] = g.edata['node_attn'] + dist_attn.reshape(len(g.edata['node_attn']),-1,1) + path_attn.reshape(len(g.edata['node_attn']),-1,1)  # Size: [num_edges, 4, 1]
...
class TripletEmbeddingCopoly(nn.Module):
    ...
    def forward(self, node_h, edge_h, fp_1, md_1, fp_2, md_2, fp_3, md_3, ratio, indicators):
        triplet_h = torch.cat([node_h, edge_h], dim=-1)
        triplet_h = self.in_proj(triplet_h)
        ...
        triplet_h[indicators==7] = self.fp_proj(fp_3)
        triplet_h[indicators==8] = self.md_proj(md_3)
        triplet_h[indicators == 9] = self.embedding(ratio[:, 2].long())

        triplet_h[indicators==10] = self.global_node_emb.weight
        return triplet_h
...
class LiGhTPredictor(nn.Module):
    def __init__(self,
        ...
        self.cl_projector = nn.Sequential(
            nn.Linear(d_g_feats*4, d_g_feats),
            activation,
    ...
    def forward(self, g, fp_1, md_1, fp_2, md_2, fp_3, md_3, ratio):
        ...
        triplet_h = self.triplet_emb(node_h, edge_h, fp_1, md_1, fp_2, md_2, fp_3, md_3, ratio, indicators)
        ...
        with g.local_scope():
            ...
            fp_vn_3 = triplet_h[indicators == 7]
            md_vn_3 = triplet_h[indicators == 8]
            ratio_vn_3 = triplet_h[indicators == 9]

            fp_vn = (fp_vn_1 + fp_vn_2 + fp_vn_3) / 3
            md_vn = (md_vn_1 + md_vn_2 + md_vn_3) / 3
            ratio_vn = (ratio_vn_1 + ratio_vn_2 + ratio_vn_3) / 3
            g.remove_nodes(np.where(indicators.detach().cpu().numpy() >= 1)[0])
            readout = dgl.readout_nodes(g, 'ht', op=self.readout_mode)
            g_feats = torch.cat([fp_vn, md_vn, ratio_vn, readout], dim=-1)

        return self.node_predictor(triplet_h[g.ndata['mask']>=1]), \
            self.fp_predictor(triplet_h[indicators==1]), self.md_predictor(triplet_h[indicators==2]), \
            self.fp_predictor(triplet_h[indicators==4]), self.md_predictor(triplet_h[indicators==5]), \
            self.fp_predictor(triplet_h[indicators==7]), self.md_predictor(triplet_h[indicators==8]), \
            self.cl_projector(g_feats)

    def forward_tune(self, g, fp_1, md_1, fp_2, md_2, fp_3, md_3, ratio):
        indicators = g.ndata['vavn'] # 0 indicates normal atoms and nodes (triplets); -1 indicates virutal atoms; >=1 indicate virtual nodes 
        # Input
        node_h = self.node_emb(g.ndata['begin_end'], indicators)          
        edge_h = self.edge_emb(g.ndata['edge'], indicators)
    ...
    def generate_fps(self, g, fp_1, md_1, fp_2, md_2, fp_3, md_3, ratio):
