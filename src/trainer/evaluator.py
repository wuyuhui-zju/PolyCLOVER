...
class Evaluator:
    ...
    def eval(self, y_true, y_pred, valid_ids=None):
        ...
        elif self.eval_metric == 'rmse_top5':
            return self._eval_top5_rmse(y_true, y_pred)
        elif self.eval_metric == 'rmse_top10':
            return self._eval_top10_rmse(y_true, y_pred)
        ...
        elif self.eval_metric == 'r':
            return self._eval_r(y_true, y_pred)
        else:
    ...
    def _eval_top5_rmse(self, y_true, y_pred):
        '''
            compute Top-5 RMSE averaged across tasks
            (Top-5 samples with smallest labels per task)
        '''
        rmse_list = []
        for i in range(y_true.shape[1]):
            is_labeled = y_true[:, i] == y_true[:, i]
            true_vals = y_true[is_labeled, i]
            pred_vals = y_pred[is_labeled, i]

            if len(true_vals) < 5:
                continue  # skip tasks with less than 5 valid samples

            if (self.mean is not None) and (self.std is not None):
                pred_vals = pred_vals * self.std[i] + self.mean[i]

            # sort by true labels
            sorted_idx = np.argsort(true_vals)
            true_top5 = true_vals[sorted_idx][:5]
            pred_top5 = pred_vals[sorted_idx][:5]

            rmse = np.sqrt(np.mean((true_top5 - pred_top5) ** 2))
            rmse_list.append(rmse)

        return sum(rmse_list) / len(rmse_list) if rmse_list else None

    def _eval_top10_rmse(self, y_true, y_pred):
        '''
            compute Top-10 RMSE averaged across tasks
            (Top-10 samples with smallest labels per task)
        '''
        rmse_list = []
        for i in range(y_true.shape[1]):
            is_labeled = y_true[:, i] == y_true[:, i]
            ...
            if len(true_vals) < 10:
                continue  # skip tasks with less than 10 valid samples
            ...
            # sort by true labels
            sorted_idx = np.argsort(true_vals)
            true_top10 = true_vals[sorted_idx][:10]
            pred_top10 = pred_vals[sorted_idx][:10]

            rmse = np.sqrt(np.mean((true_top10 - pred_top10) ** 2))
            rmse_list.append(rmse)
    ...
    def _eval_r(self, y_true, y_pred):
        '''
            compute Pearson correlation averaged across tasks
        '''
        corr_list = []
        for i in range(y_true.shape[1]):
            is_labeled = y_true[:, i] == y_true[:, i]
            ...
            if len(true_vals) < 2:
                continue  # Pearson requires at least 2 samples
        ...
        return sum(corr_list) / len(corr_list) if corr_list else None
