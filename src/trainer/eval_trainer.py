...
class Trainer():
        ...
        for epoch in range(1, self.args.n_epochs + 1):
            ...
            if self.local_rank == 0:
                ...
                if self.result_tracker.update(np.mean(best_val_result), np.mean(val_result)):
                    ...
                    best_epoch = epoch
                    final_metric = self.eval(model, test_loader, is_test=True)
                    if self.args.save:
                        torch.save(model.state_dict(),
                                   os.path.join("../models", self.args.dataset, f"best_model_{self.args.save_suffix}.pth"))

                if epoch - best_epoch >= 20:
                    break
        return best_train_result, best_val_result, best_test_result, final_metric
    ...
    def eval(self, model, dataloader, is_test=False):
        ...
        if is_test:
            result = []
            for evaluator in self.final_evaluator:
                metric = evaluator.eval(torch.cat(labels_all), torch.cat(predictions_all))
