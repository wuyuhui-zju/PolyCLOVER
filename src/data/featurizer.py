...
def smiles_to_graph(smiles_list, vocab, max_length=5, n_knowledge_nodes=2, n_global_nodes=2, add_self_loop=True):
    smiles_list = list(filter(None, smiles_list))
    edges, atom_pairs_features_in_triplets, bond_features_in_triplets, triplet_labels, virtual_atom_and_virtual_node_labels, paths, line_graph_path_labels, mol_graph_path_labels, virtual_path_labels, self_loop_labels = [], [], [], [], [], [], [], [], [], []
    for i, smiles in enumerate(smiles_list):
        edges_new, atom_pairs_features_in_triplets_new, bond_features_in_triplets_new, triplet_labels_new, virtual_atom_and_virtual_node_labels_new, paths_new, line_graph_path_labels_new, mol_graph_path_labels_new, virtual_path_labels_new, self_loop_labels_new = construct_single_component(smiles, vocab, max_length=5, n_knowledge_nodes=n_knowledge_nodes, add_self_loop=True, start_idx=max([v for edge in edges for v in edge])+1 if len(edges) > 0 else 0, index=i)
        ...
        triplet_labels.extend(triplet_labels_new)
...
def construct_single_component_tune(smiles, max_length=5, n_knowledge_nodes=2, start_idx=0, index=0, add_self_loop=True):
